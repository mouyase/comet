...
describe('skills', () => {
  ...
  describe('English Comet workflow safeguards', () => {
    it('matches the Chinese workflow decision-point requirements', async () => {
      ...
      expect(enOpen).toContain(
        'Do not run `comet openspec new change` or create proposal/design/tasks while the resolved brief or name remains ambiguous',
      );
      expect(enOpen).toContain(
  ...
  describe('Comet script discovery helper', () => {
    ...
    it('keeps archive step 2 on a single authoritative command path in both languages', async () => {
      ...
      expect(zhCommands.trim()).toBe('comet archive "<change-name>"');
      expect(enCommands.trim()).toBe('comet archive "<change-name>"');
