...
3. First run `comet openspec status --change "<name>" --json` to resolve `<openspec-change-dir>`, then read `<openspec-change-dir>/.comet/subagent-progress.md` to recover the current task or final review, implementation commit, RED/GREEN evidence, passed reviews, unresolved feedback, and review-fix round
