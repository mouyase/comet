...
- `comet-five-phase-overlay` primary state comes only from the resolved `<openspec-change-dir>/.comet.yaml`; first resolve the change directory with `comet openspec status --change "<name>" --json`. It may be `openspec/changes/<name>` (legacy) or `docs/openspec/changes/<name>` (docs). No active change or multiple active changes must block and ask the user to choose.
