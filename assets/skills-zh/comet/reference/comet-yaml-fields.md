...
本文件是每个 change 级 `.comet.yaml` 状态文件的字段参考，位置为 `<openspec-change-dir>/`。`<openspec-change-dir>` 可能是 `openspec/changes/<name>`（legacy）或 `docs/openspec/changes/<name>`（docs）。按需查阅，不随 skill 一次性加载。项目级默认配置放在 `.comet/config.yaml`，全局默认配置放在 `~/.comet/config.yaml`；项目配置优先于全局配置。
